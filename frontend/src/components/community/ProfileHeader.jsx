        ...
        <div className="flex flex-col items-center text-center gap-4">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 text-left">
                    <h1 className="text-xl font-black text-slate-900 dark:text-white">{nickname}</h1>
                    {bestBadge && (
            ...
            {((favoriteCharacter && !favoriteCharacter.img_url) || bio) && (
                <div className="flex w-full flex-col items-center gap-2">
                    {favoriteCharacter && !favoriteCharacter.img_url && (
                        <div className="inline-flex items-center gap-2 rounded-xl border-2 border-slate-200 dark:border-white/20 bg-slate-100 dark:bg-black/30 backdrop-blur-sm px-3 py-1.5">
                            <span className="text-xs font-black text-slate-800 dark:text-white">{favoriteCharacter.name}</span>
                        </div>
                    )}
                    {bio && (
                        <>
                            ...
                            <div className="flex w-full max-w-xs items-center gap-2.5">
                                <span className="h-px flex-1 bg-slate-300 dark:bg-white/25" style={tierAccent ? { background: `${tierAccent}66` } : undefined} />
                                <div
                                    className="h-3.5 w-5 shrink-0 rounded-[3px] border border-slate-300 dark:border-white/40"
                                    style={{
                            ...
                            <p className="max-w-sm text-sm text-slate-600 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{bio}</p>
                        </>
                    )}
                </div>
            )}
