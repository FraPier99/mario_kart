...
//
// L'header profilo che ospita queste chip ha ora sempre uno sfondo foto
// (vedi CircuitBackdrop) — la chip resta leggibile su qualunque foto con un
// riempimento scuro traslucido + blur, non più legato al tema chiaro/scuro
// del sito.
//
...
const BadgeChip = ({ image, medallion, title, subtitle, opacity = 1, accentColor }) => (
    <div
        className="flex shrink-0 items-center gap-2 rounded-xl bg-black/40 backdrop-blur-sm py-1.5 pr-3 pl-1.5"
        style={accentColor ? {
    ...
    >
        ...
        <div className="flex flex-col leading-tight">
            <span className="text-[11px] font-black uppercase tracking-wide text-white">{title}</span>
            {subtitle && (
                <span className="text-[10px] normal-case text-slate-300">{subtitle}</span>
            )}
        </div>
    </div>
)
