...
import { Database, PencilLine, Plus, RotateCcw, Search, Trash2, UserSquare2, ChevronDown, Trophy, Sparkles, Star, Crown, Upload, X as XIcon } from 'lucide-react'
...
const ImageUpload = ({ value, onChange }) => {
  ...
  return (
    <div className="space-y-2.5">
      <p className="text-xs font-black uppercase tracking-[0.25em] text-slate-300">Foto profilo</p>
      <div className="flex items-center gap-4">
        ...
        <div
          ...
          className={`relative h-20 w-20 shrink-0 cursor-pointer select-none rounded-full border-2 border-dashed transition outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 ${
            isDragging ? 'border-emerald-400 bg-emerald-400/10' : 'border-white/20 hover:border-emerald-400/60'
          }`}
        ...
        >
          {value ? (
            ...
            <div className="flex h-full w-full flex-col items-center justify-center gap-1">
              <UserSquare2 size={20} className="text-slate-500" />
              <p className="text-[8px] font-black uppercase tracking-wider text-slate-500">Carica</p>
            </div>
          )}
        </div>
          ...
          <div className="flex flex-wrap gap-3">
            {value && (
              <button type="button" onClick={() => onChange('')}
                className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-rose-400 transition hover:text-rose-300">
                <XIcon size={10} /> Rimuovi
              </button>
            )}
            <button type="button" onClick={() => setShowUrlInput((v) => !v)}
              className="text-[9px] font-black uppercase tracking-widest text-slate-400 transition hover:text-emerald-300">
          ...
          {showUrlInput && (
            <div className="flex gap-2">
              <input
                ...
                placeholder="https://..."
                className="flex-1 rounded-xl border border-white/10 bg-slate-950 px-3 py-1.5 text-xs text-white placeholder:text-slate-600 outline-none focus:border-emerald-400"
              />
  ...
  return (
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-200 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
          ...
          <div className="w-full max-w-sm rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl animate-scale-in"
            ...
            <p className="text-lg font-black text-white text-center">Eliminare {confirmDelete.nickname?.toUpperCase()}?</p>
            <p className="mt-2 text-sm text-slate-400 text-center">Questa azione non può essere annullata.</p>
            <div className="mt-6 flex gap-3">
              ...
              <button onClick={() => executeDelete(confirmDelete)}
                className="flex-1 rounded-2xl bg-rose-500 px-4 py-2.5 text-sm font-black text-white transition hover:bg-rose-400">
                Elimina
              </button>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-slate-950 px-4 py-4 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="rounded-3xl border-2 border-white/20 bg-linear-to-br from-slate-900 via-slate-900 to-emerald-950 p-4" style={{ boxShadow: 'var(--circuit-shadow-md)' }}>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl space-y-2">
              <div className="font-title inline-flex items-center gap-2 rounded-full border-2 border-emerald-400/40 bg-emerald-400/10 px-3 py-1.5 text-[9px] tracking-wide text-emerald-300">
              ...
              <div className="space-y-1">
                ...
                <p className="max-w-2xl text-xs leading-5 text-slate-300 sm:text-sm">
            ...
            <button
              type="button"
              onClick={refresh}
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2.5 text-xs font-bold text-white transition hover:bg-white/10"
            >
        ...
        <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
          <section className="flex flex-col rounded-3xl border border-white/10 bg-slate-900 p-4 shadow-xl shadow-black/20">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="space-y-1.5 text-sm font-medium text-slate-300">
                  ...
                  <input
                    ...
                    required
                    className="w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-2.5 text-white placeholder:text-slate-500 outline-none transition focus:border-emerald-400"
                  />
                </label>
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-xl shadow-black/20">
              ...
              {loading ? (
                ...
                <div className="overflow-hidden rounded-2xl border border-white/10">
                  {/* Vista mobile: cards */}
                  <div className="max-h-105 overflow-y-auto divide-y divide-white/10 md:hidden">
                    ...
                    <div className="min-w-0">
                      <p className="font-black text-white truncate uppercase">{player.first_name} {player.last_name}</p>
                      <p className="text-xs font-bold text-emerald-300 uppercase">{player.nickname}</p>
                    </div>
                          </div>
                    ...
                    <img
                      ...
                      className="h-10 w-10 rounded-xl border border-white/10 object-contain bg-slate-800"
                    />
                          </div>
                          <div className="flex gap-2">
                            <button
                ...
                <>
                  <div className="mt-4 grid max-h-105 gap-2 overflow-y-auto sm:grid-cols-3 xl:grid-cols-4">
                    {visibleCatalogCharacters.map((character) => (
                      <div key={character.id} className="flex flex-col border border-white/10 bg-slate-950 p-2.5 hover-lift hover:border-amber-500/30 rounded-xl">
                        <img
                          src={resolveCharacterImage(character)}
                          alt={character.name}
                          className="mb-2 h-20 w-full shrink-0 object-contain bg-slate-800"
                        />
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="text-base font-black text-white uppercase">{character.name}</p>
