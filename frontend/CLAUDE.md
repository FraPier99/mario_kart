...
- The "special profile card" gold/blue styling (highlighted border+background, avatar ring, icon badge overlay — `ProfileDashboard.jsx`, `CommunityUserPage.jsx`, `components/layout/Hero.jsx`, `components/layout/Header.jsx`) is driven by `getProfileCardStyle(bestBadge?.tier)` (`lib/playerBadges.js`), not by a binary "has won a tournament" flag — only `leggenda`/`campione`/`veterano` return a style (Crown/gold-shimmer, Trophy/gold, Star/blue respectively), everything else (including no badge) returns `null` and falls back to the normal, non-highlighted look. Superadmins have no `Player`/badges — they keep a fixed gold ("leggenda"-style) treatment as a separate `isSuperadmin` fallback, combined with `cardStyle` via `effectiveCardStyle = cardStyle ?? (isSuperadmin ? PROFILE_CARD_STYLES.leggenda : null)`. When adding a new component with this same "champion card" look, reuse this helper rather than re-deriving a boolean from `tournamentWins`.
