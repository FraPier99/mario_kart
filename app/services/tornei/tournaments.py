...
def maybe_finalize_classic_tournament(db: Session, tournament_id: int) -> None:
    """
    Se un torneo classic ha spareggi (1°/2°, 3°/4° e/o posizioni più basse,
    anche a N>2 giocatori) e questi sono stati tutti risolti, conclude
    automaticamente il torneo: determina il vincitore dalla classifica finale
    riordinata, imposta winner_id/status, salda le schedine (con assegnazione
    automatica delle Card premio) e notifica i partecipanti.

    Chiamata dopo la registrazione di un risultato in una gara di spareggio
    podio (group_name che inizia con "duello_podio_").
    """
    tournament = db.query(Tournament).filter(Tournament.id == tournament_id).first()
    if not tournament or tournament.tournament_format != "classic":
        return
    if tournament.winner_id is not None or tournament.status == "concluso":
        return

    ties = get_classic_podium_ties(db, tournament_id)
    top2, top4, others = ties.get("top2"), ties.get("top4"), ties.get("others", [])
    all_ties = [t for t in (top2, top4, *others) if t is not None]

    # Nessuno spareggio rilevato: nulla da concludere qui.
    if not all_ties:
        return

    # Se uno spareggio è ancora in corso, attendi il prossimo risultato.
    if any(t["order"] is None for t in all_ties):
        return

    order = get_classic_final_classifica(db, tournament_id)
    if not order:
        return

    tournament.winner_id = order[0]
    tournament.status = "concluso"
    settle_tournament_schedine(db, tournament_id)
    db.commit()
    db.refresh(tournament)

    winner = db.query(Player).filter(Player.id == order[0]).first()
    winner_name = winner.nickname if winner else "—"
...
def maybe_finalize_group_stage_tournament(db: Session, tournament_id: int) -> None:
    """
    Se un torneo a gironi ha spareggi podio di Finale (1°/2° e/o 3°/4°, anche
    a N>2 giocatori) e questi sono stati tutti risolti, conclude
    automaticamente il torneo: determina il vincitore dalla classifica della
    Finale riordinata, imposta winner_id/status, salda le schedine deluxe
    (con assegnazione automatica delle Card premio) e notifica i partecipanti.

    Chiamata dopo la registrazione di un risultato in una gara di spareggio
    podio della Finale (group_name in {FINALS_DUELLO_PODIO_1_2, FINALS_DUELLO_PODIO_3_4}).
    """
    tournament = db.query(Tournament).filter(Tournament.id == tournament_id).first()
    if not tournament or tournament.tournament_format != "group_stage":
        return
    if tournament.winner_id is not None or tournament.status == "concluso":
        return
    ...
    # Nessuno spareggio podio di Finale rilevato: nulla da concludere qui.
    if not all_ties:
        return
    ...
    order = get_finals_final_classifica(db, tournament_id)
    if not order:
        return
