...
- **Tournament lifecycle**: `services/tornei/tournaments.py` — by far the largest file. Tournament CRUD, group layout (`compute_group_layout`), girone/semifinal seeding and advancement (`seed_group_stage`, `_advance_top_n`, `_group_standings`), all tie-break detection/resolution (`_all_tied_blocks`, `_resolve_podium_tie`, `_first_to_n_order`, `get_classic_podium_ties`, `get_finals_podium_ties`), and the two auto-finalize entry points (`maybe_finalize_classic_tournament`, `maybe_finalize_group_stage_tournament`).
- **Results**: `services/tornei/results.py` — `create_result`/`update_result` are the only places that should call `_maybe_finalize_after_duel` (see root gotchas). Scoring uses `app/data/punteggi.py`'s `PUNTEGGI_CONFIG[n_players][position-1]`.
- **Schedine settlement**: `services/schedine/schedine.py` (classic) and `schedine_deluxe.py` (group_stage) — both `settle_*` functions score every open schedina, grant the Master card to the schedina winner (and ties), grant the Blue Shell to the actual last (and, with 7+ players, second-to-last) finisher, create `PremioTorneo` rows, and send notifications. They are *not* auto-invoked by SQLAlchemy events — every caller (`update_tournament`, `set_tournament_playoff_winner`, `maybe_finalize_*_tournament`) must explicitly dispatch to the format-correct settle function.
