...
- **Statistiche aggregate**: `services/tornei/stats.py` + `controllers/tornei/stats.py` (`/stats/*`) — leaderboard per torneo, confronto testa a testa fra due giocatori filtrato per `game_id` (self-join su `Result` sullo stesso `race_id`), e classifiche aggregate per circuito (lista leggera + dettaglio con ranking completo, per il fetch lazy on-expand del frontend). Escludono sempre `Race.is_duello.is_(True)`, come `get_leaderboard`. Tutte le route di questo controller sono pubbliche (nessun `Depends(get_current_user)`), come le altre GET di sola lettura in `circuits.py`/`results.py`/`tournaments.py`.
