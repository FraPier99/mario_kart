...
class SchedinaTorneoGroupStage(Base):
    ...
    classifica_finale_ordinata = Column(
        JSON, nullable=False
    )  # [player_id, ...] in ordine 1°→ultimo
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "tournament_id", name="unique_schedina_deluxe_per_torneo"
        ),
        CheckConstraint(
            "spareggio_distanza >= 0", name="check_deluxe_spareggio_non_negative"
        ),
