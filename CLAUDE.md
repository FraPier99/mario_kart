...
- **Tie-break duels**: classic uses `get_classic_podium_ties`/`maybe_finalize_classic_tournament`; group_stage's Finale uses `get_finals_podium_ties`/`maybe_finalize_group_stage_tournament` (both in `app/services/tornei/tournaments.py`). Group-stage *qualification* ties (girone/semifinal cutoff) are a third, different mechanism: `_resolve_tie_with_spareggio` (single race, no best-of-N).
...
- **The duel-finalize hook is easy to lose.** `app/services/tornei/results.py` must call `_maybe_finalize_after_duel` after every `create_result`/`update_result` so that `maybe_finalize_classic_tournament`/`maybe_finalize_group_stage_tournament` actually run when a tie-break duel resolves. This wiring has been silently dropped by file rewrites more than once — if duels stop auto-concluding tournaments, check this first.
